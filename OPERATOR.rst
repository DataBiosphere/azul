...
Note that the scan may trigger a WAF alarm even if the rate limit is configured
properly. This is because of two mechanisms: first, the ``PUT /manifest/files``
...
After a scan has completed, use the following steps to save a report of the
scan results.
...
#. Navigate to the *Template* tab of the *Generate Report* window, and select
   *Modern HTML report with themse and options* from the *Template* option.

#. Navigate to the *Scope* tab, and enter a value such as "AnVIL Data Portal"
   in the *Report Title* field.

#. Under "Sites", select only the URL targeted by the scan, to avoid noise
   from third party sites such as Google typekit.

#. The *Report Name* field specifies the name of the file to be created. Enter
   a value such as "2025-01-01-anvil-data-portal.html" in this field. (Note, do
   this last, since changing the *Sites* selection will cause the *Report Name*
   field to be replaced with an autogenerated value.)
