...
class TestCartItemManager(WebServiceTestCase, DynamoTestCase):
    ...
    def test_add_cart_item(self):
        ...
        item_id = self.cart_item_manager.add_cart_item(catalog=self.catalog,
                                                       user_id=user_id,
                                                       cart_id=cart_id,
                                                       entity_id='entity_id',
    ...
    def test_delete_cart_item(self):
        ...
        user_id = '111'
        cart_id = self.cart_item_manager.create_cart(user_id, 'test cart', False)
        item_id1 = self.cart_item_manager.add_cart_item(catalog=self.catalog,
                                                        user_id=user_id,
                                                        cart_id=cart_id,
                                                        entity_id='1',
                                                        entity_type='entity_type',
                                                        entity_version='entity_version')
        item_id2 = self.cart_item_manager.add_cart_item(catalog=self.catalog,
                                                        user_id=user_id,
                                                        cart_id=cart_id,
                                                        entity_id='2',
                                                        entity_type='entity_type',
    ...
    def test_cart_item_batch_write_unauthorized(self):
        ...
        cart_id = self.cart_item_manager.create_cart(user_id, 'test cart', False)
        with self.assertRaises(ResourceAccessError):
            self.cart_item_manager.start_batch_cart_item_write(self.catalog, '112', cart_id, 'foo', {}, 12345, 10000)
