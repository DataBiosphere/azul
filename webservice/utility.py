...
# if you want to tune billings, this is the dial. 1.2 means add 20%
# on top of what is calculated
EXTRA_MONEY = 1.2
...
def calculate_compute_cost(total_seconds, vm_cost_hr):
    return Decimal(Decimal(total_seconds)/Decimal(SECONDS_IN_HR)*Decimal(
        vm_cost_hr)*Decimal(EXTRA_MONEY))


def calculate_storage_cost(portion_month_stored, file_size_gb):
    return Decimal(portion_month_stored)*Decimal(file_size_gb)*Decimal(
        STORAGE_PRICE_GB_MONTH)*Decimal(EXTRA_MONEY)


def get_vm_string(host_metrics):
    return str(host_metrics.get("vm_region")) + str(
        host_metrics.get("vm_instance_type"))


def make_bills(comp_aggregations, previous_month_bytes, portion_of_month,
               this_month_timestamps_sizes, curr_time, seconds_in_month):
    x = comp_aggregations
    print(x)
    instances = x["aggregations"][
        "filtered_nested_timestamps"]["filtered_range"]["vmtype"]["buckets"]
    ...
    for instance in instances:
        ...
        for region in regions:
            ...
            print(regionName,
                  instanceType,
                  totalTime,
                  pricing[regionName+instanceType])
            total_pricing += calculate_compute_cost(
                totalTime, pricing[regionName + instanceType])

    # need to get the storage size for files completed
    #  before start of this month
    storage_size_bytes = previous_month_bytes[
        'aggregations'][
        'filtered_nested_timestamps']['sum_sizes']['value']
    storage_size_gb = Decimal(storage_size_bytes)/Decimal(BYTES_IN_GB)
    total_pricing += Decimal(
        STORAGE_PRICE_GB_MONTH
    )*storage_size_gb*portion_of_month*Decimal(EXTRA_MONEY)

    # calculate the money spent on storing workflow
    # outputs which were uploaded during this month
    this_month_timestamps = this_month_timestamps_sizes[
        'aggregations']['filtered_nested_timestamps']['times'][
        'buckets']
...
def get_compute_costs(comp_aggregations):
    ...
    instances = comp_aggregations["aggregations"][
        "filtered_nested_timestamps"]["filtered_range"]["vmtype"]["buckets"]
    ...
    for instance in instances:
        ...
        for region in regions:
            regionName = region["key"]
            totalTime = region["totaltime"]["value"]
            print(regionName, instanceType, totalTime,
                  pricing[regionName+instanceType])
            compute_costs += calculate_compute_cost(
                totalTime,
                pricing[regionName + instanceType])
            ...
            for sample in specimen.get("samples"):
                for analysis in sample.get("analysis"):
                    ...
                    if timing_stats:
                        ...
                        analysis_end_time = get_datetime_from_es(
                            timing_stats["overall_stop_time_utc"])
                        analysis_start_time = get_datetime_from_es(
                            timing_stats["overall_start_time_utc"])
                        if analysis_end_time \
                                < bill_time_end \
                                and analysis_start_time >= bill_time_start:
                            host_metrics = analysis.get("host_metrics")
                            if host_metrics:
                                cost = calculate_compute_cost(
                                    time,
                                    pricing.get(get_vm_string(host_metrics)))
...
def create_storage_costs_json(project_files_hits,
                              bill_time_start,
                              bill_time_end,
                              month_total_seconds):
                ...
                for analysis in sample.get("analysis"):
                    ...
                    if timing_stats:
                        ...
                        if analysis_end_time < bill_time_end:
                            this_size = get_gb_size(
                                workflow_output_total_size(
                                    analysis.get("workflow_outputs")))
                            # means it's from this month
                            if analysis_end_time >= bill_time_start:
                                seconds = (
                                    bill_time_end - analysis_end_time
                                ).total_seconds()
                            # it's from previous month,
                            # charge it portion of month
                            else:
                                seconds = (
                                    bill_time_end - bill_time_start
                                ).total_seconds()
                            cost = calculate_storage_cost(
                                Decimal(seconds)/Decimal(month_total_seconds
                                                         ), this_size)
...
def get_storage_costs(previous_month_bytes,
                      portion_of_month,
                      this_month_timestamps_sizes,
                      curr_time, seconds_in_month):
    storage_costs = Decimal(0)
    storage_size_bytes = previous_month_bytes[
        'aggregations']['filtered_nested_timestamps'][
        'sum_sizes']['value']
    storage_size_gb = Decimal(storage_size_bytes)/Decimal(BYTES_IN_GB)
    storage_costs += calculate_storage_cost(portion_of_month, storage_size_gb)

    # calculate the money spent on storing workflow outputs
    # which were uploaded during this month
    this_month_timestamps = this_month_timestamps_sizes[
        'aggregations']['filtered_nested_timestamps']['times'][
        'buckets']
    for ts_sum in this_month_timestamps:
        ...
        storage_costs += calculate_storage_cost(month_portion,
                                                storage_size_gb)
