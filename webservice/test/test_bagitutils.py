...
class TestBagHandlerMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # bag.write_csv_files method of the firecloud_tsv_mock.tsv file residing in
        # the test folder.

        self.bag_manifest = 'test/manifest_bag'
        self.bag_participant_tsv = 'test/manifest_bag/participants.tsv'
        self.bag_sample_tsv = 'test/manifest_bag/samples.tsv'
        self.data = "Program	Project	Center Name	Submitter Donor ID	Donor UUID	Submitter Donor Primary Site	Submitter Specimen ID	Specimen UUID	Submitter Specimen Type	Submitter Experimental Design	Submitter Sample ID	Sample UUID	Analysis Type	Workflow Name	Workflow Version	File Type	File Path	Upload File ID	Data Bundle UUID	Metadata.json	File URLs	File DOS URL\n\
        ...
        self.data2 = ("Program	Project	Center Name	Submitter Donor ID	Donor UUID	Submitter Donor Primary Site	Submitter Specimen ID	Specimen UUID	Submitter Specimen Type	Submitter Experimental Design	Submitter Sample ID	Sample UUID	Analysis Type	Workflow Name	Workflow Version	File Type	File Path	Upload File ID	Data Bundle UUID	Metadata.json	File URLs	File DOS URI\n\
NIH Data Commons	NIH Data Commons Pilot	Broad Public Datasets	ABC123456	c2b4c298-4d80-4aaa-bddf-20c15d184af3	Blood	NA12878_2	bfcc3266-340a-5751-8db1-d661163ac8e5	Normal - Blood	Seq_DNA_SNP_CNV; Seq_DNA_WholeGenome	H06JUADXX130110_1	c774934f-4100-44bf-8df9-8d4e509c088d	none	test workflow	Development	bam	H06JUADXX130110.1.ATCACGAT.20k_reads.bam	60936d97-6358-4ce3-8136-d5776186ee21	dd04fbf3-2a51-4c72-8038-da7094b8da55		gs://broad-public-datasets/NA12878_downsampled_for_testing/unmapped/H06JUADXX130110.1.ATCACGAT.20k_reads.bam	dos://dos-dss.ucsc-cgp-dev.org/ga4gh/dos/v1/dataobjects/60936d97-6358-4ce3-8136-d5776186ee21?version=2018-03-23T123738.145535Z")
        # TSV file has only one row with a CRAM file.
        self.data3 = ("Donor UUID	Sample UUID	File Type	Specimen UUID	File URLs	File DOS URI\n"
                      "d2	s3	cram	sp1	gs://junk.cram	dos://abc?d5f")

    def test_zipRootIsManifest(self):
        bag = BagHandler(data=self.data, bag_info={}, bag_name='manifest')
    ...
    def testDemoData(self):
        bag = BagHandler(data=self.data2, bag_info={}, bag_name='manifest')
        ...
        self.assertEquals(row['gs_url2'], 'gs://broad-public-datasets/NA12878_downsampled_for_testing/unmapped/H06JUADXX130110.1.ATCACGAT.20k_reads.bam')
        self.assertFalse('s3_url2' in row)
    ...
    def test_process_demo_data(self):
        ...
        participants, max_files_in_sample, protocols, file_types = \
        ...
        samples = bag.samples(protocols, file_types)
        self.assertEqual(len(samples), 13)

        suffixes = [bag.get_suffix(x) for x in file_types]

        # Ensure every row has file_dos_uri<suffix> column
        # for suffix in [str(i) for i in range(1, max_files_in_sample + 1)]:
        for item in range(len(suffixes)):
            for i in range(0, len(samples)):
                self.assertIn('file_dos_uri' + suffixes[item],
                              samples[i].keys())
    ...
    def test_get_suffix(self):
        bag = BagHandler(data=self.data, bag_info={}, bag_name='manifest')
        suffix = bag.get_suffix('crai')
        self.assertTrue(suffix, '1')
        suffix = bag.get_suffix('bam')
        self.assertTrue(suffix, '2')
        suffix = bag.get_suffix('fastq')
        self.assertTrue(suffix, '0')
        suffix = bag.get_suffix('FASTQ.gz')
        self.assertTrue(suffix, '0')
        self.assertRaises(ValueError, bag.get_suffix, 'not_a_filetype')

    def test_only_one_cram_in_data(self):
        """The data we're loading here only contains one row, and that row
        contains a CRAM file. We test whether "samples" has a column name
         "file_type2"."""
        bag = BagHandler(data=self.data3, bag_info={}, bag_name='manifest')
        participants, max_files_in_sample, protocols, file_types = \
            bag.participants_and_max_files_in_sample_and_protocols()
        self.assertEqual(len(participants), 1)
        self.assertEqual(len(protocols), 1)
        self.assertEqual(max_files_in_sample, 1)

        samples = bag.samples(protocols, file_types)
        self.assertFalse('file_type0' in samples[0].keys())
        self.assertFalse('file_type1' in samples[0].keys())
        self.assertEqual(len(samples[0].keys()), 9)
        self.assertEqual(len(samples), 1)
        self.assertTrue('file_type2' in samples[0].keys())
        self.assertTrue('file_dos_uri2' in samples[0].keys())
        self.assertTrue(samples[0]['gs_url2'] == 'gs://junk.cram')
        self.assertTrue(samples[0]['file_dos_uri2'] == 'dos://abc?d5f')

    def test_fc_mock(self):
        """Tests a small mock file with a minimal set of columns that covers
        the following edge cases:
            - common case of one sample of a donor with one crai and one cram
              file, repeated for two samples, that's donor "d1"
            - similar to above, but in the second sample the rows of 
              crai and cram are altered, that's donor "d2"
            - only one sample of a donor with only one cram but no crai or
              bam
            - a sample with a bai and a bam."""

        mock_simple = 'test/firecloud_tsv_mock.tsv'
        ...
        samples = bag.samples(protocols, file_types)
        self.assertEqual(len(samples[0].keys()), 16)
        ...
        self.assertTrue(set(fcmp.same_files) == set(['participants.tsv', 'samples.tsv']))
