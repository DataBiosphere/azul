...
class MyTestCase(unittest.TestCase):
    ...
    def test_transform_request(self):
        """
        Tests the transform request function from ElasticTransformDump.
        This test assumes that ElasticSearch is operational, and that
        the domain and port for ElasticSearch are provided in the form of
        environmental variables. It must be loaded with known test data
        :return: True or false depending on the assertion
        """
        ...
        actual_output = es_requester.transform_request(
        ...
        expected_output = EsTd.open_and_return_json(
            '{}/test_transform_request_test1.json'.format(base_path))
        # Convert objects to be compared to strings
        expected_output = json.dumps(expected_output, sort_keys=True)
    ...
    def test_file_autocomplete_request(self):
        """
        Tests the file autocomplete request function from
        ElasticTransformDump. This test assumes that ElasticSearch is
        operational, and that the domain and port for ElasticSearch
        are provided in the form of environmental
        ...
        es_requester = EsTd(
            ...
            es_port=es_port,
            es_protocol=es_protocol)
        actual_output = es_requester.transform_autocomplete_request(
            pagination, request_config_file=request_config,
            mapping_config_file=mapping_config,
            _query='dc', search_field='fileId',
            entry_format='file')
        expected_output = EsTd.open_and_return_json(
            '{}/test_autocomplete_request_test1.json'.format(base_path))
        # Convert objects to be compared to strings
        expected_output = json.dumps(expected_output, sort_keys=True)
        actual_output = json.dumps(actual_output, sort_keys=True)
        # Print the 2 strings for reference
    ...
    def test_file_donor_autocomplete_request(self):
        """
        Tests the donor autocomplete request function from
        ElasticTransformDump. This test assumes that ElasticSearch
        is operational, and that the domain and port for
        ElasticSearch are provided in the form of environmental
        variables. It must be loaded with known test data
        :return: True or false depending on the assertion
        """
        # Get the parameters to be used to the EsTd
        ...
        expected_output = EsTd.open_and_return_json(
            '{}/test_donor_autocomplete_request_test1.json'.format(base_path))
        # Convert objects to be compared to strings
        expected_output = json.dumps(expected_output, sort_keys=True)
        actual_output = json.dumps(actual_output, sort_keys=True)
        # Print the 2 strings for reference
    ...
    def test_summary_request(self):
        """
        Tests the summary request function. This test assumes that
        ElasticSearch is operational, and that the domain and port
        for ElasticSearch are provided in the form of environmental
        variables. It must be loaded with known test data
        :return: True or false depending on the assertion
        """
        ...
        es_requester = EsTd(
            ...
            es_port=es_port,
            es_protocol=es_protocol)
        actual_output = es_requester.transform_summary(
            request_config_file=request_config,
            filters=sample_filter)
        expected_output = EsTd.open_and_return_json(
            '{}/test_summary_request_test1.json'.format(base_path))
        # Convert objects to be compared to strings
        expected_output = json.dumps(expected_output, sort_keys=True)
        actual_output = json.dumps(actual_output, sort_keys=True)
        print "Comparing the two dictionaries built."
    ...
    def test_manifest_request(self):
        """
        Tests the summary request function. This test assumes
        that ElasticSearch is operational, and that the domain
        and port for ElasticSearch are provided in the form of environmental
        variables. It must be loaded with known test data
        :return: True or false depending on the assertion
        """
        # Getting the request config file and setting up the filters
        ...
        es_requester = EsTd(
            ...
            es_port=es_port,
            es_protocol=es_protocol)
        actual_output = es_requester.transform_manifest(
            request_config_file=request_config,
            filters=sample_filter)
        ...
        with open('{}/test_manifest_request_test1.tsv'.format(
                base_path)) as expected_file:
