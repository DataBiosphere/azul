...
In order to index metadata stored in a [Terra Data Repository] (TDR) instance,
the Google service account provisioned by Terraform must be registered with SAM 
and authorized for repository read access for datasets and snapshots.

The registration is handled automatically during ``make deploy``. To register 
without deploying, run ``make -C terraform sam``. The authorization is currently 
done manually by someone on the Azul team. Ask on Slack to find out more.
