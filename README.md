...
- [3. Deployment](#3-deployment)
    ...
    - [3.3 Locating REST API endpoints via DNS](#33-locating-rest-api-endpoints-via-dns)
...
Some Terraform configuration is generated by `make deploy`, but the rest is
...
to these files require running `make terraform` again in order to update
cloud infrastructure for the selected deployment.

If you are provisioning multiple deployments, you will need to run `make deploy`
before `make terraform` for each one, since the Terraform configuration
generated by `make deploy` is deployment specific.

## 3.3 Locating REST API endpoints via DNS
