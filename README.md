...
5. Install the development prerequisites:
   ...
   installing [pyenv] and then Python 3.9 using `pyenv install 3.9.18` and
   setting `PYENV_VERSION` to `3.9.18`. You may need to update pyenv itself
...
Set `PYENV_VERSION` to `3.9.18` in `environment.local.py` at the project root.
...
```
(.venv) ~/workspace/hca/azul$ docker run -it -v ${project_root}/:/root/azul python:3.9.18-bullseye bash
