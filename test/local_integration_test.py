...
class IntegrationTest(unittest.TestCase):
    ...
    prefix_length = 2
    queue_empty_timeout = 4500 / (8 ** (prefix_length - 1)) if prefix_length <= 3 else 60
    ...
    def check_bundles_are_indexed(self, test_name: str, entity_type: str, indexed_bundle_fqids: Set[str]):
        ...
        logger.info('Starting integration test %s with the prefix %s for the entity type %s. Expected %s bundle(s). ',
                    test_name, self.bundle_uuid_prefix, entity_type, num_bundles)
        logger.debug('Expected Bundle IDs: %s ', sorted(indexed_bundle_fqids))
        ...
        logger.info('Checking if bundles are in the service responses...')
        retries = 0
        ...
        while retries < max_num_of_retries:
            ...
            ids_with_prefix = {bundle_id for bundle_id in found_bundle_fqids
                               if bundle_id.startswith(self.bundle_uuid_prefix)}
            ...
            logger.info('On attempt #%i/%i, found %i out of %s bundles. There are a total of %s hits.'
                        ' The current page has %s hits in %s.', retries+1, max_num_of_retries, len(ids_with_prefix),
                        num_bundles, total_entities, len(hits), url)

            if search_after is None and search_after_uid is None:
                if ids_with_prefix == indexed_bundle_fqids:
        ...
        self.assertEqual(ids_with_prefix, indexed_bundle_fqids)
