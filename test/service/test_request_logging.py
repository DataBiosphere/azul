    ...
    def test_request_logs(self):
        for level, response_log in [
            ...
            for auth_status, auth_stmt in [
                ("unauthenticated", ""),
                ("authenticated", " with token 'foo_token'")
            ]:
                with self.subTest(level=level, auth=auth_status):
                    with self.assertLogs(logger=log, level=level) as logs:
                        url = self.base_url + '/health/basic'
                        headers = {'Authorization': 'Bearer foo_token'} if auth_status == 'authenticated' else {}
                        requests.get(url, headers=headers)
                    logs = [(r.levelno, r.getMessage()) for r in logs.records]
                    self.assertEqual(logs, [
                        (INFO, f"Received {auth_status} GET request to '/health/basic' without parameters{auth_stmt}."),
                        (level, response_log)
                    ])
