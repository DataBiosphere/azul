...
class TestHCAIndexer(IndexerTestCase):
    ...
    def _assert_old_bundle(self, num_expected_new_contributions=0, ignore_aggregates=False, ignore_deletes=False):
        ...
        hits = self._get_hits()
        entity_contributions = 5
        self.assertEqual(entity_contributions + entity_contributions + num_expected_new_contributions, len(hits))
        hits_by_id = {}
        for hit in hits:
            entity_type, aggregate = config.parse_es_index_name(hit['_index'])
    ...
    def _assert_new_bundle(self, num_expected_old_contributions=0, old_hits_by_id=None):
        ...
        hits = self._get_hits()
        entity_contributions = 5
        self.assertEqual(entity_contributions + entity_contributions + num_expected_old_contributions, len(hits))
        for hit in hits:
            entity_type, aggregate = config.parse_es_index_name(hit['_index'])
            source = hit['_source']
            version = one(source['bundles'])['version'] if aggregate else source['bundle_version']
        ...
        for hit in hits:
            ...
            if entity_type == 'files':
                ...
                if aggregate:
                    ...
                    self.assertEqual(2, one(contents['files'])['count'])
                else:
                    self.assertEqual(2, len(contents['files']))
            else:
                self.fail()
    ...
    def test_indexing_with_skipped_matrix_file(self):
        ...
        self.assertEqual(3, len(entities_with_matrix_files))  # a project and a specimen
