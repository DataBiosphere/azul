...
class TestHCAIndexer(IndexerTestCase):
    ...
    def test_no_cell_count_contributions(self):
        def assert_cell_suspension(expected_result: JSON, hits: List[JSON]):
            def is_project(hit) -> bool:
                return ('projects', True) == self._parse_index_name(hit)

            project_hit = one(filter(is_project, hits))
            cell_suspension = one(project_hit['_source']['contents']['cell_suspensions'])
            actual_result = {
                field: cell_suspension[field]
                for field in {'total_estimated_cells', 'total_estimated_cells_', 'organ_part'}
            }
        ...
        # Project metadata is changed here to ensure that the project aggregate
        # has cell suspensions from both bundles.
