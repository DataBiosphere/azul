...
page_size=100000
if len(sys.argv) < 2:
	print ("USAGE: "+sys.argv[0]+" <template file> {number of entries per call (default=100000)} {number of iterations (default=1)}")
	sys.exit(1)
...
subprocess.run("curl -XPUT $ES_PROTOCOL://$ES_SERVICE:$ES_PORT/test_data/_mapping/meta?update_all_types  -d @td_mapping.json", shell=True)
print("\n\nLoading data")
index = 1
for j in range(0, numIts):
	...
	subprocess.run("curl -s -XPUT $ES_PROTOCOL://$ES_SERVICE:$ES_PORT/test_data/_bulk?pretty --data-binary @td_index_page.jsonl >/dev/null", shell=True)
	print()
