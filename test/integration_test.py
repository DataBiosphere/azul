...
class IntegrationTestCase(AzulTestCase, metaclass=ABCMeta):
    ...
    def _list_partition_bundles(self,
        ...
        partition_prefixes = list(prefix.effective.partition_prefixes())
        partition_prefix = self.random.choice(partition_prefixes)
        effective_prefix = source.spec.prefix.common + partition_prefix
    ...
    def test_indexing(self):
        ...
        class Catalog:
            ...
            def notifications_with_duplicates(self) -> List[JSON]:
                ...
                return notifications + self.random.choices(notifications, k=num_duplicates)
        ...
        if index:
            for catalog in catalogs:
                self.azul_client.queue_notifications(catalog.notifications_with_duplicates())
            _wait_for_indexer()
            for catalog in catalogs:
