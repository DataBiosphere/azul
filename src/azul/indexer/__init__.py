...
@attrs.frozen(kw_only=True, eq=False, hash=True)
...
class SourcedBundleFQID(BundleFQID, Generic[SOURCE_REF]):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        ...     foo: str
        """
        # Ideally this would go in the parent class, but it's pushed down here
        # to facilitate the assertion in :meth:`__eq__`
        super().__init_subclass__(**kwargs)
        assert cls.__eq__ is SourcedBundleFQID.__eq__, cls
        assert cls.__hash__ is SourcedBundleFQID.__hash__, cls

    def __hash__(self):
        # Since we define a custom `__eq__` we're forced to also define a custom
        # `__hash__`, even though both implementations just call super()
        return super().__hash__()

    def __eq__(self, other: 'BundleFQID') -> bool:
        same_bundle = super().__eq__(other)
        if same_bundle and isinstance(other, SourcedBundleFQID):
            assert self.source == other.source, (self, other)
        return same_bundle
