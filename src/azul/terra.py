...
class UserAuthTDRClient(TDRClient):
    ...
    def list_snapshots(self) -> JSONs:
        ...
        # instance of TerraClient obtains a fresh token, making token-based
        # caching ineffective between lambda invocations. In order for the cache
        # to be effective, this class should be provided with a token that will
        # be used consistently throughout a user's session, such as those
        # generated by the Swagger UI.
        token = self.credentials.token
        assert not isinstance(token, ServiceAccountCredentials)
