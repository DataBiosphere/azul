...
from .metadata_api.bundle import (Bundle,
                                  EntityVisitor,
                                  Process,
                                  LibraryPreparationProcess,
                                  SequencingProcess,
                                  Entity,
                                  Biomaterial,
                                  UUID4,
                                  ...
                                  DonorOrganism,
                                  SequenceFile, CellSuspension)
from azul.dss_bundle import DSSBundle

log = logging.getLogger(__name__)


def _project_dict(bundle: Bundle) -> dict:
...
def _specimen_dict(
    ...
    specimen_list = []
    for specimen in (s for s in biomaterials.values() if isinstance(s, SpecimenFromOrganism)):
...
class TransformerVisitor(EntityVisitor):
    specimens: MutableMapping[UUID4, SpecimenFromOrganism]
    processes: MutableMapping[str, Any]  # Merges process with protocol
    files: MutableMapping[UUID4, Any]  # Merges manifest + file metadata

    @staticmethod
    def _merge_process_protocol(pc: Process, pl: Protocol) -> MutableMapping[str, Any]:
    ...
    def visit(self, entity: Entity) -> None:
        if isinstance(entity, SpecimenFromOrganism):
...
class BiomaterialVisitor(EntityVisitor):
    ...
    def visit(self, entity: Entity) -> None:
        ...
        if isinstance(entity, Biomaterial):
            ...
            elif isinstance(entity, CellSuspension):
