        ...
        for file in self.bundle.files.values():
            ...
            if not is_zarr or sub_name.endswith('.zattrs'):
                ...
                contents = dict(samples=list(map(self._sample, samples.values())),
                                ...
                                analysis_protocols=list(
                                    map(self._analysis_protocol, visitor.analysis_protocols.values())
                                ),
                                imaging_protocols=list(
                                    map(self._imaging_protocol, visitor.imaging_protocols.values())
                                ),
                                library_preparation_protocols=list(
                                    map(
                                        self._library_preparation_protocol,
                                        visitor.library_preparation_protocols.values())
                                ),
                                sequencing_protocols=list(
                                    map(self._sequencing_protocol, visitor.sequencing_protocols.values())
                                ),
