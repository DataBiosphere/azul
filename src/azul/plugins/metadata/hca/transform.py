...
class BaseTransformer(Transformer, metaclass=ABCMeta):
    ...
    # REVIEW: If this works, I would like to move this to api.File

    def _is_matrix(self, file: api.File):
        return any('matrix' in c.lower() for c in file.content_description)
    ...
    def transform(self) -> Iterable[Contribution]:
        ...
        for file in files:
            ...
            if not is_zarr or sub_name.endswith('.zattrs'):
                ...
                if isinstance(file, api.SupplementaryFile) and self._is_matrix(file):
