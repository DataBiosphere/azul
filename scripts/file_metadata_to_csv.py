...
class Flatten:
    ...
    def dump(self, filename='output.csv', delim=","):
        self.write_tsv(filename, delim, [y for x in self.all_objects_by_project_id.values() for y in x])

    def dump_by_project(self, filename='output.csv', file_directory='', delim=','):
        for project_uuid, objects in self.all_objects_by_project_id.items():
            os.makedirs(os.path.join(file_directory, project_uuid), exist_ok=True)
            self.write_tsv(os.path.join(file_directory, project_uuid, filename), delim, objects)

    def write_tsv(self, filename, delim, objects):
...
def convert_bundle_dirs():
    ...
    if args.project:
        flattener.dump_by_project(filename=args.filename, delim=args.seperator)
    else:
